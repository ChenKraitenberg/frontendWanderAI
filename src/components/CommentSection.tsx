...
const CommentSection: React.FC<CommentSectionProps> = ({ postId, initialComments = [], commentCount, onCommentAdded }) => {
  ...
  // Add this inside CommentSection component
  const updateCommentsWithLatestUserInfo = useCallback(() => {
    if (!comments.length) return;

    // Get all unique user IDs in the comments
    const userIds = new Set(comments.map((comment) => comment.user._id));
    let hasUpdates = false;

    // Create a new comments array to avoid direct state mutation
    const updatedComments = [...comments];

    userIds.forEach((commentUserId) => {
      // If this is the current user
      if (commentUserId === userId && userName) {
          ...
          if (comment.user._id === userId) {
            if (comment.user.name !== userName) {
              updatedComments[index] = {
                ...
                user: {
                  ...comment.user,
                  name: userName || '', // Ensure name is never undefined
                  avatar: userAvatar || comment.user.avatar,
                },
              };
              hasUpdates = true;
            }
    ...
    if (hasUpdates) {
      ...
      // Also update the post's comments on the server
      if (postId) {
        // Make sure all comments have non-undefined names and convert Date objects to strings before sending to service
        const commentsForService = updatedComments.map((comment) => ({
          ...comment,
          createdAt: comment.createdAt instanceof Date ? comment.createdAt.toISOString() : comment.createdAt,
          user: {
            ...comment.user,
            name: comment.user.name || 'Anonymous', // Ensure name is never undefined
          },
        }));

        postService.updatePostComments(postId, commentsForService).catch((err) => console.error('Failed to sync comment updates:', err));
      }
    }
  }, [comments, userId, userName, userAvatar, postId]);

  // Call this on component mount and whenever comments change
  useEffect(() => {
    updateCommentsWithLatestUserInfo();
  }, [updateCommentsWithLatestUserInfo]);
