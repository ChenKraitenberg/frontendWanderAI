...
const PostCard: React.FC<PostCardProps> = ({ post, onPostClick, onLike, onCommentClick, onDelete, onEdit, showActions = true }) => {
  ...
  // Function to update current post with latest user info from localStorage
  const updatePostWithLatestUserInfo = useCallback(() => {
    // Only update the user info if the post belongs to the current user
    if (post.userId === currentUserId && post.user) {
      const storedName = localStorage.getItem('userName');
      const storedAvatar = localStorage.getItem('userAvatar');
      ...
      // For posts by other users, use the post as is
      setCurrentPost(post);
    }
  }, [post, currentUserId]);
  ...
  useEffect(() => {
    const refreshPostData = async () => {
      try {
        if (post && post._id) {
          const updatedPost = await postService.getPostById(post._id);
          if (updatedPost) {
            // Only update the user info for the current user's posts
            if (updatedPost.userId === currentUserId && updatedPost.user) {
              const storedName = localStorage.getItem('userName');
              const storedAvatar = localStorage.getItem('userAvatar');
    ...
    refreshPostData();
  }, [post._id, post, refreshTrigger, currentUserId]);
  ...
  const getUserAvatar = () => {
    // If the user is the current logged-in user, use potentially updated avatar from localStorage
    if (currentPost.user?._id === currentUserId) {
      const storedAvatar = localStorage.getItem('userAvatar');
      if (storedAvatar) {
        // Don't add timestamp to data URLs
        if (storedAvatar.startsWith('data:')) {
